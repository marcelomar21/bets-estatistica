/**
 * Centralized configuration from environment variables
 */
require('dotenv').config();

const config = {
  // Supabase
  supabase: {
    url: process.env.SUPABASE_URL,
    serviceKey: process.env.SUPABASE_SERVICE_KEY,
  },

  // Telegram
  telegram: {
    botToken: process.env.TELEGRAM_BOT_TOKEN,
    adminGroupId: process.env.TELEGRAM_ADMIN_GROUP_ID,
    publicGroupId: process.env.TELEGRAM_PUBLIC_GROUP_ID,
  },

  // External APIs
  apis: {
    theOddsApiKey: process.env.THE_ODDS_API_KEY,
    openaiApiKey: process.env.OPENAI_API_KEY,
    footystatsApiKey: process.env.FOOTYSTATS_API_KEY,
  },

  // Environment
  env: process.env.NODE_ENV || 'development',
  isDev: process.env.NODE_ENV !== 'production',

  // Bot mode: 'central' (only central jobs), 'group' (only group jobs), 'mixed' (all)
  botMode: process.env.BOT_MODE || 'mixed',

  // Betting rules
  betting: {
    minOdds: 1.60,
    maxActiveBets: 50,
    maxDaysAhead: 2, // 2 dias conforme PRD (FR39)
    validBookmakerDomains: [
      'bet365.com',
      'betano.com',
      'betano.com.br',
      'betano.bet.br',
      'betway.com',
      'sportingbet.com',
    ],
  },

  // Jobs toggle
  jobs: {
    enrichOdds: process.env.ENRICH_ODDS_ENABLED === 'true', // disabled: replaced by /odds-collector (Playwright + Betano)
  },

  // Retry configuration
  retry: {
    maxAttempts: 3,
    baseDelayMs: 1000,
  },

  // Membership configuration (Story 16.4, 16.5)
  // Tech-Spec: Migração MP - trial now managed by MP, keep config for visibility
  // Story 3.1: Multi-tenant - GROUP_ID identifica qual grupo este bot atende
  membership: {
    groupId: process.env.GROUP_ID || null,
    trialDays: (() => {
      const days = parseInt(process.env.MEMBERSHIP_TRIAL_DAYS || '2', 10);
      if (isNaN(days) || days <= 0) {
        return 2; // Default fallback
      }
      return days;
    })(),
    // Grace period: days to wait before kicking inadimplente members
    gracePeriodDays: 2,
    // MP manages trial and checkout, but we still reference for notifications
    checkoutUrl: process.env.MP_CHECKOUT_URL || process.env.CAKTO_CHECKOUT_URL || null,
    operatorUsername: 'marcelomar1121',
    subscriptionPrice: process.env.MEMBERSHIP_SUBSCRIPTION_PRICE || 'R$50/mes',
  },

  // Mercado Pago configuration
  mercadoPago: {
    accessToken: process.env.MP_ACCESS_TOKEN || null,
    webhookSecret: process.env.MP_WEBHOOK_SECRET || null,
    checkoutUrl: process.env.MP_CHECKOUT_URL || null,
    webhookPort: parseInt(process.env.MP_WEBHOOK_PORT, 10) || 3001,
  },

  // WhatsApp integration
  whatsapp: {
    enabled: process.env.WHATSAPP_ENABLED === 'true',
    encryptionKey: process.env.WHATSAPP_ENCRYPTION_KEY,
    rateLimitMsgsPerMin: parseInt(process.env.WHATSAPP_RATE_LIMIT || '10', 10),
    healthCheckIntervalMs: 60000,
    qrCodeExpireMs: 60000,
    reconnectBackoffMs: [1000, 5000, 15000, 30000, 60000],
    maxReconnectAttempts: 5,
    shutdownTimeoutMs: 30000,
    maxNumbersPerGroup: 3,
    poolWarnThreshold: 5,
  },

  // LLM configurations (não são segredos, são constantes)
  llm: {
    // Modelo pesado: análise de jogos, avaliação de resultados
    heavyModel: 'gpt-5.4',
    // Modelo leve: copy, interpretação de mercado
    lightModel: 'gpt-5.4-nano',
    // Aliases para retrocompatibilidade
    get resultEvaluatorModel() { return this.heavyModel; },
  },
};

// Validation
function validateConfig() {
  // Skip validation in test environment (tests mock supabase)
  if (process.env.NODE_ENV === 'test' || process.env.SKIP_CONFIG_VALIDATION === 'true') {
    return;
  }

  const required = [
    ['SUPABASE_URL', config.supabase.url],
    ['SUPABASE_SERVICE_KEY', config.supabase.serviceKey],
  ];

  // Telegram vars required only when WhatsApp-only mode is NOT active
  if (!config.whatsapp.enabled || config.telegram.botToken) {
    required.push(
      ['TELEGRAM_BOT_TOKEN', config.telegram.botToken],
      ['TELEGRAM_ADMIN_GROUP_ID', config.telegram.adminGroupId],
      ['TELEGRAM_PUBLIC_GROUP_ID', config.telegram.publicGroupId],
    );
  }

  // WhatsApp-specific required vars (only when enabled)
  if (config.whatsapp.enabled) {
    required.push(['WHATSAPP_ENCRYPTION_KEY', config.whatsapp.encryptionKey]);
  }

  const missing = required.filter(([_name, value]) => !value);

  if (missing.length > 0) {
    const names = missing.map(([name]) => name).join(', ');
    throw new Error(`Missing required environment variables: ${names}`);
  }
}

/**
 * Load per-group configs from database (Phase 2: multi-group support)
 * Reads from bot_pool (source of truth for tokens/IDs) JOIN groups (per-group config)
 * @param {object} supabaseClient - Supabase client instance
 * @returns {Promise<Map<string, object>>} Map of groupId -> groupConfig
 */
async function loadGroupConfigs(supabaseClient) {
  const { data, error } = await supabaseClient
    .from('bot_pool')
    .select(`
      bot_token,
      bot_username,
      group_id,
      admin_group_id,
      public_group_id,
      is_active,
      groups!inner (
        id,
        name,
        posting_schedule,
        max_active_bets,
        copy_tone_config,
        status
      )
    `)
    .eq('is_active', true)
    .eq('groups.status', 'active');

  if (error) {
    console.error('[config] Failed to load group configs:', error.message);
    return new Map();
  }

  const configs = new Map();
  for (const row of (data || [])) {
    configs.set(row.group_id, {
      botToken: row.bot_token,
      botUsername: row.bot_username,
      adminGroupId: row.admin_group_id,
      publicGroupId: row.public_group_id,
      groupName: row.groups.name,
      postingSchedule: row.groups.posting_schedule,
      maxActiveBets: row.groups.max_active_bets ?? config.betting.maxActiveBets,
      copyToneConfig: row.groups.copy_tone_config || {},
    });
  }

  return configs;
}

module.exports = { config, validateConfig, loadGroupConfigs };
