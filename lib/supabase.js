...
// Validate on module load (skipped in test environment)
validateConfig();

// Create singleton client (use dummy values in test - tests mock this module)
const supabase = createClient(
  config.supabase.url || 'http://localhost:54321',
  config.supabase.serviceKey || 'test-key',
  {
    auth: {
...
async function testConnection() {
  try {
    const { data: _data, error } = await supabase.rpc('get_service_status', {}).maybeSingle();
