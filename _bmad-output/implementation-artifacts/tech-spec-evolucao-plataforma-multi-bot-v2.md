...
tech_stack:
  ...
  - LangChain 1.1.x + OpenAI (GPT-4o, GPT-4o-mini, o4-mini) + Anthropic (Claude) + Moonshot (Kimi)
...
- Severidade: ALTA
- `trackResults.js` usa GPT-4o via `evaluateBetsWithLLM()` para determinar resultado
...
- Hoje: 1 chamada GPT-4o decide acerto/erro (não-determinístico, pode alucinar)
- Proposta do usuário: usar 3 LLMs independentes
  - Se as 3 concordam → resultado confirmado
  - Se há divergência → step adicional de confirmação (ex: LLM diferente com mais contexto, ou flag para revisão manual)
- Aumenta custo de API mas reduz drasticamente erros de avaliação
- Considerar: GPT-4o + Claude + Gemini? Ou 3x o mesmo com temperature variada?
...
- `copyService.js` usa `gpt-4o-mini` com `temperature: 0.2`, `maxTokens: 200`
...
4. **Consenso multi-LLM para resultados (3 provedores distintos)**:
   - `resultEvaluator.js`: rodar 3 chains em paralelo: **GPT-5.1-mini** (OpenAI) + **Claude** (Anthropic) + **Kimi** (Moonshot)
...
5. **Recovery sweep para tracking**:
   - Novo job diário (ex: 01:00) que busca `bet_status='posted' AND bet_result='pending' AND kickoff_time < now-6h`
...
7. **Preview + edição de mensagens (mobile-first, só web)**:
   - Novo endpoint `POST /api/bets/post-now/preview` que gera copy sem enviar, retorna `previewId` + textos em cache (TTL 30min)
   - UI mobile-first: cards empilhados full-width, um por aposta, com ações Editar/Regenerar/Remover
   - "Regenerar" mostra diff visual do que mudou (comparação com cache do `previewId`)
...
- [ ] **Task 1.1: Diagnosticar e corrigir Guru offline (B1 + B2)**
  ...
  - Notes: Pode ser fix de config sem mudança de código. Osmar funciona = código ok, problema é infra/config.
...
- [ ] **Task 1.5: Expandir janela de tracking + adicionar recovery sweep (B4)**
  ...
  - Action:
    ...
    3. Adicionar recovery sweep: novo bloco no final de `runTrackResults()` que busca `bet_status='posted' AND bet_result='pending' AND kickoff_time < now-8h` e tenta avaliar com dados finais
  - Notes: O recovery sweep garante que nenhuma bet é permanentemente perdida. Matches que demoraram pra completar serão avaliados no ciclo seguinte.
...
- [ ] **Task 2.1: Criar BotContext e BotRegistry**
  ...
  - Action:
    ...
    3. `initBots()` substitui `initBot()`: lê todos os grupos ativos + bot_pool do Supabase, cria 1 `TelegramBot` instance por token
...
- [ ] **Task 2.2: Refatorar config.js para multi-group**
  ...
  - Action:
    ...
    2. Adicionar `loadGroupConfigs()` que lê da tabela `groups` + `bot_pool` no startup
    3. Cada grupo carrega: token, adminGroupId, publicGroupId, postingSchedule, copyToneConfig, maxActiveBets
...
- [ ] **Task 3.2: Implementar consenso multi-LLM para avaliação de resultados**
  ...
  - Action:
    1. Criar 3 chains LLM com o mesmo schema Zod, usando **provedores distintos** para diversidade arquitetural real:
       - `evaluatorChainA`: **GPT-5.1-mini** (OpenAI, temp 0) — via LangChain `ChatOpenAI`
       - `evaluatorChainB`: **Claude** (Anthropic — Sonnet 4.6 ou Haiku 4.5, temp 0) — via LangChain `ChatAnthropic`
       - `evaluatorChainC`: **Kimi** (Moonshot, temp 0) — via LangChain `ChatOpenAI` com baseURL customizada
    2. Executar as 3 em `Promise.allSettled()` (paralelo, tolerante a falha individual)
    3. Lógica de consenso:
       - 3/3 concordam → resultado confirmado com `confidence: 'high'`
       - 2/3 concordam → resultado da maioria com `confidence: 'medium'`
       - 3 divergem → `result: 'unknown'`, `confidence: 'low'`, flag para revisão manual
    4. Salvar `result_confidence` no campo criado na migration (Task 2.3)
  ...
  - Notes: Três provedores completamente distintos = se um modelo tem viés de treinamento, os outros dois compensam. Custo controlado: GPT-5.1-mini e Haiku são econômicos, e multi-LLM só roda para mercados complexos (~20% das apostas). Requer API keys: `ANTHROPIC_API_KEY` e `MOONSHOT_API_KEY` adicionados ao Render.
...
- [ ] **Task 4.1: API endpoint para tom de voz**
  ...
  - Notes: Seguir padrão de tenant middleware existente (`createApiHandler`). O PUT aceita tanto o formato estruturado (campos individuais) quanto o texto livre (que é processado por LLM antes de salvar).
...
- [ ] **Task 4.5: UI — Fluxo de Preview + Edição no admin panel**
  ...
  - Action: Fluxo **mobile-first** com cards empilhados:
    ...
    3. Cada card tem 3 ações (thumb-zone friendly):
       ...
       - **"Regenerar"** — re-chama LLM via endpoint regenerate, mostra **diff visual** do que mudou
...
- [ ] **Task 5.6: Remover backward-compat e cleanup**
  ...
  - Notes: Só executar depois que o deploy unificado estiver validado.
...
- [ ] AC 3.3: Given uma aposta de mercado complexo (ex: "Handicap Asiático -0.5") avaliada por 3 LLMs (GPT-5.1-mini, Claude, Kimi) que concordam, when o tracking salva, then `result_confidence = 'high'`.
...
- [ ] AC 5.5: Given o cleanup da Task 5.6 executado, when o código é auditado, then nenhuma referência a `config.telegram.adminGroupId` ou `config.telegram.publicGroupId` existe fora de backward-compat.
...
| Supabase Management API | Fase 2 | Infra | Aplicar migration consolidada (029) |
...
|---|---|---|
| Multi-LLM consensus aumenta custo | Financeiro | Validação determinística para mercados simples (~80% dos casos); multi-LLM (GPT-5.1-mini + Claude + Kimi) só para mercados complexos (~20%) |
...
| Preview/edição gera latência perceptível (N chamadas LLM) | UX | Loading state progressivo, gerar previews em paralelo, cache com `previewId` (TTL 30min) |
| Concorrência: 2 admins preparando postagem ao mesmo tempo | Dados | `previewId` isolado por sessão + `post_now_requested_at` com optimistic update |
| Kimi (Moonshot) API instável ou com alta latência | Operacional | `Promise.allSettled` tolera falha de 1 provider; se Kimi falha, consenso é entre os 2 restantes |
...
| 2 | Multi-LLM com GPT-5.1-mini + Claude + Kimi | Marcelomendes | Provedores distintos com capacidade de raciocínio real; modelos "lixosos" descartados |
...
| Fase 2 — Fundação BotContext | 3 tasks | Média | Não |
