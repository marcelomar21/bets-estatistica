...
- O job `node scripts/check_analysis_queue.js [--dry-run] [--window-hours=N]` garante que só os jogos que são o próximo compromisso **dos dois times** permaneçam na fila.  
  ...
  - `dados_importados`: `daily_update` já trouxe stats/lastx frescos.
  - `analise_completa`: `runAnalysis` finalizou e há JSON intermediário.
  - `relatorio_concluido`: `agent/persistence` salvou Markdown/DB e o PDF pode ser gerado com segurança.
...
graph LR
  ...
  C -->|analise_completa| D[agent/persistence]
  D -->|relatorio_concluido| E[Relatórios/DB]
  C -.->|falha → pending| A
  B -.->|fila vazia| A
```
...
3. `node agent/analysis/runAnalysis.js today` – agora lê a fila; só executa se houver jogos `dados_importados` e, ao terminar cada jogo, marca como `analise_completa`.
4. `node agent/persistence/main.js <match_id>` – após gerar Markdown/JSON, marca o `match_id` como `relatorio_concluido` na fila, preenchendo `analysis_generated_at`.
