...
app.post('/api/preview', async (req, res) => {
  ...
  const { group_id, bet_id, bet_ids } = req.body || {};
  if (!group_id) {
  ...
  try {
    ...
    const result = await generatePreview(group_id, bet_id || null, Array.isArray(bet_ids) ? bet_ids : null);
...
async function setupScheduler() {
  ...
  if (runCentral) {
    ...
    // Generate daily hit art - 09:00 São Paulo (GURU-19)
    cron.schedule('0 9 * * *', async () => {
  ...
  if (runCentral) {
    ...
    console.log('   09:00 - Generate daily hit art (GURU-19)');
