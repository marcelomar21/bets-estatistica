...
describe('assignBetToGroup (junction table)', () => {
  it('INSERTs into bet_group_assignments', async () => {
    const chain = {
      ...
      select: jest.fn().mockResolvedValue({
        data: [{ id: 'bga-1', bet_id: 'bet-1', group_id: 'group-a', posting_status: 'ready', post_at: '10:00', created_at: '2026-03-28T12:00:00Z' }],
        error: null,
      }),
    };
    ...
    expect(chain.upsert).toHaveBeenCalledWith(
      { bet_id: 'bet-1', group_id: 'group-a', posting_status: 'ready', post_at: '10:00' },
  ...
  it('does not include post_at when null', async () => {
    const chain = {
      upsert: jest.fn().mockReturnThis(),
      select: jest.fn().mockResolvedValue({
        data: [{ id: 'bga-1', bet_id: 'bet-1', group_id: 'group-a', posting_status: 'ready', post_at: null, created_at: '2026-03-28T12:00:00Z' }],
        error: null,
      }),
    };
    const updateChain = {
    ...
    expect(chain.upsert).toHaveBeenCalledWith(
      { bet_id: 'bet-1', group_id: 'group-a', posting_status: 'ready' },
      { onConflict: 'bet_id,group_id', ignoreDuplicates: true }
    );
  });
});
