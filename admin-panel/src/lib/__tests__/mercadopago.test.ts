...
import { createSubscriptionPlan, updateSubscriptionPlanPrice } from '../mercadopago';
...
describe('updateSubscriptionPlanPrice', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.MERCADO_PAGO_ACCESS_TOKEN = 'test-token';
  });
  ...
  it('returns error when access token is not configured', async () => {
    ...
    const result = await updateSubscriptionPlanPrice('plan-123', 39.9);
  ...
  it('returns error when price is zero or negative', async () => {
    const resultZero = await updateSubscriptionPlanPrice('plan-123', 0);
    expect(resultZero).toEqual({ success: false, error: 'Preço deve ser maior que zero' });

    const resultNeg = await updateSubscriptionPlanPrice('plan-123', -5);
    expect(resultNeg).toEqual({ success: false, error: 'Preço deve ser maior que zero' });

    expect(mockFetch).not.toHaveBeenCalled();
  });

  it('sends PUT with correct payload and returns success', async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ id: 'plan-123', status: 'active' }),
    });
    ...
    expect(mockFetch).toHaveBeenCalledTimes(1);
    expect(mockFetch).toHaveBeenCalledWith(
      'https://api.mercadopago.com/preapproval_plan/plan-123',
      expect.objectContaining({
        method: 'PUT',
        headers: expect.objectContaining({
          Authorization: 'Bearer test-token',
    ...
    const body = JSON.parse((mockFetch.mock.calls[0][1] as RequestInit).body as string);
    expect(body.auto_recurring.transaction_amount).toBe(39.9);
  });
